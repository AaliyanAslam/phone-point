...
const ProductPage = async ({ params }) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 text-center">
        <div>
          ...
          <Link href="/" className="text-purple-600 mt-4 inline-block hover:underline">Return Home</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen bg-gray-50 flex flex-col items-center py-4 sm:py-10 px-2 xs:px-4 sm:px-6 lg:px-8 ${inter.className}`}>
      
      {/* BACK BUTTON - Better UX for mobile */}
      <div className="w-full max-w-6xl mb-4">
        <Link href="/" className="inline-flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-purple-700 transition-colors">
          ...
          <div className="lg:col-span-7 p-5 xs:p-8 sm:p-10 md:p-14 bg-linear-to-br from-[#2C0741] via-[#4B2050] to-[#171127] text-white flex flex-col justify-center">
            ...
            <div className="flex flex-wrap items-end gap-3 xs:gap-5 mb-6 xs:mb-8">
              <div className="flex flex-col">
                <span className="text-gray-400 text-[10px] xs:text-xs uppercase font-bold tracking-tighter">Current Price</span>
              ...
              {isDeal && mobile.oldPrice && (
                <div className="flex flex-col pb-0.5 opacity-60">
                  <span className="text-gray-400 text-[9px] xs:text-[10px] uppercase font-bold line-through">Was</span>
            ...
            <div className="space-y-5 xs:space-y-8 mb-8 xs:mb-10">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 xs:gap-2">
                  {mobile.features?.map((f, i) => (
                    <span key={i} className="bg-white/10 border border-white/10 px-2.5 py-1 xs:px-3 xs:py-1.5 rounded-lg text-[10px] xs:text-xs sm:text-sm whitespace-nowrap">
                      {f}
                    </span>
                  ))}
                </div>
              ...
              {/* Description */}
              <div>
                <h3 className="text-purple-300 font-bold text-xs xs:text-sm lg:text-base mb-2">Description</h3>
                <p className="text-gray-300 leading-relaxed text-xs xs:text-sm sm:text-base 2xl:text-lg max-w-2xl">
                  {mobile.description || "Premium high-performance mobile device with cutting-edge technology and sleek design."}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs xs:text-sm sm:text-base">
                ...
                <span>{mobile.stock > 0 ? `Ready in Stock (${mobile.stock})` : "Currently Out of Stock"}</span>
              </div>
            </div>
